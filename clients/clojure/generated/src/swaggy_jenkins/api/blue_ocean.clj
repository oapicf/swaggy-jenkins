...
(defn get-json-web-key-with-http-info
  "
...
(defn get-json-web-key
  "
  Retrieve JSON Web Key"
  [key ]
  (:data (get-json-web-key-with-http-info key)))

(defn get-json-web-token-with-http-info
  "
  Retrieve JSON Web Token"
  ([] (get-json-web-token-with-http-info nil))
  ([{:keys [expiry-time-in-mins max-expiry-time-in-mins ]}]
   (call-api "/jwt-auth/token" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"expiryTimeInMins" expiry-time-in-mins "maxExpiryTimeInMins" max-expiry-time-in-mins }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    []})))

(defn get-json-web-token
  "
  Retrieve JSON Web Token"
  ([] (get-json-web-token nil))
  ([optional-params]
   (:data (get-json-web-token-with-http-info optional-params))))
...
(defn get-scm-organisation-repositories-with-http-info
  ...
  ([organization scm scm-organisation {:keys [credential-id page-size page-number ]}]
   (check-required-params organization scm scm-organisation)
...
(defn get-scm-organisation-repository-with-http-info
  ...
  ([organization scm scm-organisation repository {:keys [credential-id ]}]
   (check-required-params organization scm scm-organisation repository)
