...
bool PipelineRunNode::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t(U("edges"))))
    {
        ...
        if(!fieldValue.is_null())
        {
            std::vector<std::shared_ptr<PipelineRunNodeedges>> refVal_edges;
...
bool PipelineRunNode::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    ...
    {
        std::vector<std::shared_ptr<PipelineRunNodeedges>> refVal_edges;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(U("edges"))), refVal_edges );
        setEdges(refVal_edges);
    }
