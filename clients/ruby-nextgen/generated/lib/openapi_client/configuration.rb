...
module OpenapiClient
  class Configuration
    attr_accessor :base_url, :timeout, :logger, :debugging, :query_array_encoding, :username, :password, :api_key
    ...
    def apply_auth(headers, query, auth_names)
      if username && auth_names.include?('jenkins_auth')
        headers['Authorization'] = Faraday::Utils.basic_header_from(username, password)
      end
      headers['Authorization'] = api_key if api_key && auth_names.include?('jwt_auth')
      nil
    end
