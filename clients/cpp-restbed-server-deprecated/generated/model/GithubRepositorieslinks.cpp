/**
 * Swaggy Jenkins
 * Jenkins API clients generated from Swagger / Open API specification
 *
 * The version of the OpenAPI document: 3.4.1-pre.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.24.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */



#include "GithubRepositorieslinks.h"

#include <string>
#include <vector>
#include <sstream>
#include <stdexcept>
#include <boost/property_tree/ptree.hpp>
#include <boost/property_tree/json_parser.hpp>

using boost::property_tree::ptree;
using boost::property_tree::read_json;
using boost::property_tree::write_json;

namespace org {
namespace openapitools {
namespace server {
namespace model {

GithubRepositorieslinks::GithubRepositorieslinks(boost::property_tree::ptree const& pt)
{
        fromPropertyTree(pt);
}

GithubRepositorieslinks::~GithubRepositorieslinks() = default;

std::string GithubRepositorieslinks::toJsonString(bool prettyJson /* = false */)
{
    return toJsonString_internal(prettyJson);
}

void GithubRepositorieslinks::fromJsonString(std::string const& jsonString)
{
    fromJsonString_internal(jsonString);
}

boost::property_tree::ptree GithubRepositorieslinks::toPropertyTree()
{
    return toPropertyTree_internal();
}

void GithubRepositorieslinks::fromPropertyTree(boost::property_tree::ptree const& pt)
{
    fromPropertyTree_internal(pt);
}

std::string GithubRepositorieslinks::toJsonString_internal(bool prettyJson)
{
	std::stringstream ss;
	write_json(ss, this->toPropertyTree(), prettyJson);
	return ss.str();
}

void GithubRepositorieslinks::fromJsonString_internal(std::string const& jsonString)
{
	std::stringstream ss(jsonString);
	ptree pt;
	read_json(ss,pt);
	this->fromPropertyTree(pt);
}

ptree GithubRepositorieslinks::toPropertyTree_internal()
{
	ptree pt;
	ptree tmp_node;
	if (m_Self != nullptr) {
		pt.add_child("self", m_Self->toPropertyTree());
	}
	pt.put("_class", m__class);
	return pt;
}

void GithubRepositorieslinks::fromPropertyTree_internal(ptree const &pt)
{
	ptree tmp_node;
	if (pt.get_child_optional("self")) {
		m_Self = std::make_shared<Link>();
		m_Self->fromPropertyTree(pt.get_child("self"));
	}
	m__class = pt.get("_class", "");
}

std::shared_ptr<Link> GithubRepositorieslinks::getSelf() const
{
    return m_Self;
}

void GithubRepositorieslinks::setSelf(std::shared_ptr<Link> value)
{
	m_Self = value;
}
std::string GithubRepositorieslinks::get_Class() const
{
    return m__class;
}

void GithubRepositorieslinks::set_Class(std::string value)
{
	m__class = value;
}

std::vector<GithubRepositorieslinks> createGithubRepositorieslinksVectorFromJsonString(const std::string& json)
{
    std::stringstream sstream(json);
    boost::property_tree::ptree pt;
    boost::property_tree::json_parser::read_json(sstream,pt);

    auto vec = std::vector<GithubRepositorieslinks>();
    for (const auto& child: pt) {
        vec.emplace_back(GithubRepositorieslinks(child.second));
    }

    return vec;
}

}
}
}
}

