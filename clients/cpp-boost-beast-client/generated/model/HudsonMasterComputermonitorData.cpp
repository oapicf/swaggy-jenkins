/**
 * Swaggy Jenkins
 * Jenkins API clients generated from Swagger / Open API specification
 *
 * The version of the OpenAPI document: 3.4.1-pre.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.24.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */



#include "HudsonMasterComputermonitorData.h"

#include <cstddef>
#include <map>
#include <memory>
#include <stdexcept>
#include <sstream>
#include <string>
#include <iterator>
#include <utility>
#include <vector>
#include <boost/json.hpp>

namespace org {
namespace openapitools {
namespace client {
namespace model {

namespace {

template <typename Target>
struct JsonValueConverter
{
    static boost::json::value toJsonValue(const Target& sourceValue)
    {
        return boost::json::value_from(sourceValue);
    }

    static Target fromJsonValue(const boost::json::value& jsonValue)
    {
        return boost::json::value_to<Target>(jsonValue);
    }
};

template <>
struct JsonValueConverter<std::nullptr_t>
{
    static boost::json::value toJsonValue(std::nullptr_t)
    {
        return nullptr;
    }

    static std::nullptr_t fromJsonValue(const boost::json::value& jsonValue)
    {
        if (!jsonValue.is_null()) {
            throw std::invalid_argument("Expected a null JSON value");
        }
        return nullptr;
    }
};

template <typename ModelType>
struct JsonValueConverter<std::shared_ptr<ModelType>>
{
    static boost::json::value toJsonValue(const std::shared_ptr<ModelType>& model)
    {
        return model == nullptr ? boost::json::value(nullptr) : model->toJsonValue();
    }

    static std::shared_ptr<ModelType> fromJsonValue(const boost::json::value& jsonValue)
    {
        if (jsonValue.is_null()) {
            return nullptr;
        }
        return std::make_shared<ModelType>(jsonValue);
    }
};

template <typename Element>
struct JsonValueConverter<std::vector<Element>>
{
    static boost::json::value toJsonValue(const std::vector<Element>& sourceValues)
    {
        boost::json::array jsonValues;
        for (const auto& sourceValue : sourceValues) {
            jsonValues.emplace_back(JsonValueConverter<Element>::toJsonValue(sourceValue));
        }
        return jsonValues;
    }

    static std::vector<Element> fromJsonValue(const boost::json::value& jsonValue)
    {
        const auto& jsonValues = jsonValue.as_array();
        std::vector<Element> convertedValues;
        convertedValues.reserve(jsonValues.size());
        for (const auto& jsonElement : jsonValues) {
            convertedValues.emplace_back(JsonValueConverter<Element>::fromJsonValue(jsonElement));
        }
        return convertedValues;
    }
};

template <typename MappedValue>
struct JsonValueConverter<std::map<std::string, MappedValue>>
{
    static boost::json::value toJsonValue(const std::map<std::string, MappedValue>& sourceValues)
    {
        boost::json::object jsonValues;
        for (const auto& sourceEntry : sourceValues) {
            jsonValues[sourceEntry.first] = JsonValueConverter<MappedValue>::toJsonValue(sourceEntry.second);
        }
        return jsonValues;
    }

    static std::map<std::string, MappedValue> fromJsonValue(const boost::json::value& jsonValue)
    {
        std::map<std::string, MappedValue> convertedValues;
        for (const auto& jsonEntry : jsonValue.as_object()) {
            const std::string entryKey(jsonEntry.key().data(), jsonEntry.key().size());
            convertedValues.emplace(entryKey, JsonValueConverter<MappedValue>::fromJsonValue(jsonEntry.value()));
        }
        return convertedValues;
    }
};

void writePrettyJson(std::ostream& output, boost::json::value const& value, std::string& indent)
{
    if (value.is_object()) {
        const auto& object = value.as_object();
        if (object.empty()) {
            output << "{}";
            return;
        }

        output << "{\n";
        indent.append(4, ' ');
        for (auto iterator = object.begin(); iterator != object.end(); ++iterator) {
            output << indent << boost::json::serialize(iterator->key()) << ": ";
            writePrettyJson(output, iterator->value(), indent);
            if (std::next(iterator) != object.end()) {
                output << ',';
            }
            output << '\n';
        }
        indent.resize(indent.size() - 4);
        output << indent << '}';
        return;
    }

    if (value.is_array()) {
        const auto& array = value.as_array();
        if (array.empty()) {
            output << "[]";
            return;
        }

        output << "[\n";
        indent.append(4, ' ');
        for (auto iterator = array.begin(); iterator != array.end(); ++iterator) {
            output << indent;
            writePrettyJson(output, *iterator, indent);
            if (std::next(iterator) != array.end()) {
                output << ',';
            }
            output << '\n';
        }
        indent.resize(indent.size() - 4);
        output << indent << ']';
        return;
    }

    output << boost::json::serialize(value);
}
}

HudsonMasterComputermonitorData::HudsonMasterComputermonitorData(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string HudsonMasterComputermonitorData::toJsonString(bool prettyJson /* = false */) const
{
    const auto value = toJsonValue();
    if (!prettyJson) {
        return boost::json::serialize(value);
    }

    std::ostringstream output;
    std::string indent;
    writePrettyJson(output, value, indent);
    return output.str();
}

void HudsonMasterComputermonitorData::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value HudsonMasterComputermonitorData::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void HudsonMasterComputermonitorData::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for HudsonMasterComputermonitorData");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object HudsonMasterComputermonitorData::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_Hudson_node_monitors_SwapSpaceMonitorIsSet) {
            object["hudson.node_monitors.SwapSpaceMonitor"] = JsonValueConverter<std::shared_ptr<SwapSpaceMonitorMemoryUsage2>>::toJsonValue(getHudsonNodeMonitorsSwapSpaceMonitor());
        }
        if (m_Hudson_node_monitors_TemporarySpaceMonitorIsSet) {
            object["hudson.node_monitors.TemporarySpaceMonitor"] = JsonValueConverter<std::shared_ptr<DiskSpaceMonitorDescriptorDiskSpace>>::toJsonValue(getHudsonNodeMonitorsTemporarySpaceMonitor());
        }
        if (m_Hudson_node_monitors_DiskSpaceMonitorIsSet) {
            object["hudson.node_monitors.DiskSpaceMonitor"] = JsonValueConverter<std::shared_ptr<DiskSpaceMonitorDescriptorDiskSpace>>::toJsonValue(getHudsonNodeMonitorsDiskSpaceMonitor());
        }
        if (m_Hudson_node_monitors_ArchitectureMonitorIsSet) {
            object["hudson.node_monitors.ArchitectureMonitor"] = JsonValueConverter<std::string>::toJsonValue(getHudsonNodeMonitorsArchitectureMonitor());
        }
        if (m_Hudson_node_monitors_ResponseTimeMonitorIsSet) {
            object["hudson.node_monitors.ResponseTimeMonitor"] = JsonValueConverter<std::shared_ptr<ResponseTimeMonitorData>>::toJsonValue(getHudsonNodeMonitorsResponseTimeMonitor());
        }
        if (m_Hudson_node_monitors_ClockMonitorIsSet) {
            object["hudson.node_monitors.ClockMonitor"] = JsonValueConverter<std::shared_ptr<ClockDifference>>::toJsonValue(getHudsonNodeMonitorsClockMonitor());
        }
        if (m__classIsSet) {
            object["_class"] = JsonValueConverter<std::string>::toJsonValue(get_Class());
        }
    return object;
}

void HudsonMasterComputermonitorData::fromJsonObject_internal(boost::json::object const& object)
{
    m_Hudson_node_monitors_SwapSpaceMonitorIsSet = false;
    m_Hudson_node_monitors_TemporarySpaceMonitorIsSet = false;
    m_Hudson_node_monitors_DiskSpaceMonitorIsSet = false;
    m_Hudson_node_monitors_ArchitectureMonitorIsSet = false;
    m_Hudson_node_monitors_ResponseTimeMonitorIsSet = false;
    m_Hudson_node_monitors_ClockMonitorIsSet = false;
    m__classIsSet = false;
    {
        const auto Hudson_node_monitors_SwapSpaceMonitorIt = object.find("hudson.node_monitors.SwapSpaceMonitor");
        if (Hudson_node_monitors_SwapSpaceMonitorIt != object.end()) {
            setHudsonNodeMonitorsSwapSpaceMonitor(JsonValueConverter<std::shared_ptr<SwapSpaceMonitorMemoryUsage2>>::fromJsonValue(Hudson_node_monitors_SwapSpaceMonitorIt->value()));
        }
    }
    {
        const auto Hudson_node_monitors_TemporarySpaceMonitorIt = object.find("hudson.node_monitors.TemporarySpaceMonitor");
        if (Hudson_node_monitors_TemporarySpaceMonitorIt != object.end()) {
            setHudsonNodeMonitorsTemporarySpaceMonitor(JsonValueConverter<std::shared_ptr<DiskSpaceMonitorDescriptorDiskSpace>>::fromJsonValue(Hudson_node_monitors_TemporarySpaceMonitorIt->value()));
        }
    }
    {
        const auto Hudson_node_monitors_DiskSpaceMonitorIt = object.find("hudson.node_monitors.DiskSpaceMonitor");
        if (Hudson_node_monitors_DiskSpaceMonitorIt != object.end()) {
            setHudsonNodeMonitorsDiskSpaceMonitor(JsonValueConverter<std::shared_ptr<DiskSpaceMonitorDescriptorDiskSpace>>::fromJsonValue(Hudson_node_monitors_DiskSpaceMonitorIt->value()));
        }
    }
    {
        const auto Hudson_node_monitors_ArchitectureMonitorIt = object.find("hudson.node_monitors.ArchitectureMonitor");
        if (Hudson_node_monitors_ArchitectureMonitorIt != object.end()) {
            setHudsonNodeMonitorsArchitectureMonitor(JsonValueConverter<std::string>::fromJsonValue(Hudson_node_monitors_ArchitectureMonitorIt->value()));
        }
    }
    {
        const auto Hudson_node_monitors_ResponseTimeMonitorIt = object.find("hudson.node_monitors.ResponseTimeMonitor");
        if (Hudson_node_monitors_ResponseTimeMonitorIt != object.end()) {
            setHudsonNodeMonitorsResponseTimeMonitor(JsonValueConverter<std::shared_ptr<ResponseTimeMonitorData>>::fromJsonValue(Hudson_node_monitors_ResponseTimeMonitorIt->value()));
        }
    }
    {
        const auto Hudson_node_monitors_ClockMonitorIt = object.find("hudson.node_monitors.ClockMonitor");
        if (Hudson_node_monitors_ClockMonitorIt != object.end()) {
            setHudsonNodeMonitorsClockMonitor(JsonValueConverter<std::shared_ptr<ClockDifference>>::fromJsonValue(Hudson_node_monitors_ClockMonitorIt->value()));
        }
    }
    {
        const auto _classIt = object.find("_class");
        if (_classIt != object.end()) {
            set_Class(JsonValueConverter<std::string>::fromJsonValue(_classIt->value()));
        }
    }
}

std::shared_ptr<SwapSpaceMonitorMemoryUsage2> HudsonMasterComputermonitorData::getHudsonNodeMonitorsSwapSpaceMonitor() const
{
    return m_Hudson_node_monitors_SwapSpaceMonitor;
}

void HudsonMasterComputermonitorData::setHudsonNodeMonitorsSwapSpaceMonitor(std::shared_ptr<SwapSpaceMonitorMemoryUsage2> value)
{
    
    m_Hudson_node_monitors_SwapSpaceMonitor = std::move(value);
    m_Hudson_node_monitors_SwapSpaceMonitorIsSet = true;
}
std::shared_ptr<DiskSpaceMonitorDescriptorDiskSpace> HudsonMasterComputermonitorData::getHudsonNodeMonitorsTemporarySpaceMonitor() const
{
    return m_Hudson_node_monitors_TemporarySpaceMonitor;
}

void HudsonMasterComputermonitorData::setHudsonNodeMonitorsTemporarySpaceMonitor(std::shared_ptr<DiskSpaceMonitorDescriptorDiskSpace> value)
{
    
    m_Hudson_node_monitors_TemporarySpaceMonitor = std::move(value);
    m_Hudson_node_monitors_TemporarySpaceMonitorIsSet = true;
}
std::shared_ptr<DiskSpaceMonitorDescriptorDiskSpace> HudsonMasterComputermonitorData::getHudsonNodeMonitorsDiskSpaceMonitor() const
{
    return m_Hudson_node_monitors_DiskSpaceMonitor;
}

void HudsonMasterComputermonitorData::setHudsonNodeMonitorsDiskSpaceMonitor(std::shared_ptr<DiskSpaceMonitorDescriptorDiskSpace> value)
{
    
    m_Hudson_node_monitors_DiskSpaceMonitor = std::move(value);
    m_Hudson_node_monitors_DiskSpaceMonitorIsSet = true;
}
std::string HudsonMasterComputermonitorData::getHudsonNodeMonitorsArchitectureMonitor() const
{
    return m_Hudson_node_monitors_ArchitectureMonitor;
}

void HudsonMasterComputermonitorData::setHudsonNodeMonitorsArchitectureMonitor(std::string value)
{
    
    m_Hudson_node_monitors_ArchitectureMonitor = std::move(value);
    m_Hudson_node_monitors_ArchitectureMonitorIsSet = true;
}
std::shared_ptr<ResponseTimeMonitorData> HudsonMasterComputermonitorData::getHudsonNodeMonitorsResponseTimeMonitor() const
{
    return m_Hudson_node_monitors_ResponseTimeMonitor;
}

void HudsonMasterComputermonitorData::setHudsonNodeMonitorsResponseTimeMonitor(std::shared_ptr<ResponseTimeMonitorData> value)
{
    
    m_Hudson_node_monitors_ResponseTimeMonitor = std::move(value);
    m_Hudson_node_monitors_ResponseTimeMonitorIsSet = true;
}
std::shared_ptr<ClockDifference> HudsonMasterComputermonitorData::getHudsonNodeMonitorsClockMonitor() const
{
    return m_Hudson_node_monitors_ClockMonitor;
}

void HudsonMasterComputermonitorData::setHudsonNodeMonitorsClockMonitor(std::shared_ptr<ClockDifference> value)
{
    
    m_Hudson_node_monitors_ClockMonitor = std::move(value);
    m_Hudson_node_monitors_ClockMonitorIsSet = true;
}
std::string HudsonMasterComputermonitorData::get_Class() const
{
    return m__class;
}

void HudsonMasterComputermonitorData::set_Class(std::string value)
{
    
    m__class = std::move(value);
    m__classIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<HudsonMasterComputermonitorData>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<HudsonMasterComputermonitorData>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<HudsonMasterComputermonitorData>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<HudsonMasterComputermonitorData>>>::fromJsonValue(boost::json::parse(json));
}

}
}
}
}

