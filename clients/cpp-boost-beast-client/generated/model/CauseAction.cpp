...
CauseAction::CauseAction(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string CauseAction::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value CauseAction::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void CauseAction::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for CauseAction");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object CauseAction::toJsonObject_internal() const
{
    boost::json::object object;
        ...
        if (m_CausesIsSet) {
            object["causes"] = JsonValueConverter<std::vector<std::shared_ptr<CauseUserIdCause>>>::toJsonValue(getCauses());
        }
    return object;
}

void CauseAction::fromJsonObject_internal(boost::json::object const& object)
{
    m__classIsSet = false;
    m_CausesIsSet = false;
    {
    ...
    {
        const auto CausesIt = object.find("causes");
        if (CausesIt != object.end()) {
            setCauses(JsonValueConverter<std::vector<std::shared_ptr<CauseUserIdCause>>>::fromJsonValue(CausesIt->value()));
        }
    }
}
...
void CauseAction::setCauses(std::vector<std::shared_ptr<CauseUserIdCause>> value)
{
    ...
    m_Causes = std::move(value);
    m_CausesIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<CauseAction>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<CauseAction>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<CauseAction>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<CauseAction>>>::fromJsonValue(boost::json::parse(json));
}
