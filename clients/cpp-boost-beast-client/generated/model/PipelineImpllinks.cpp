...
PipelineImpllinks::PipelineImpllinks(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value PipelineImpllinks::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void PipelineImpllinks::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for PipelineImpllinks");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object PipelineImpllinks::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_SelfIsSet) {
            object["self"] = JsonValueConverter<std::shared_ptr<Link>>::toJsonValue(getSelf());
        }
        if (m_ActionsIsSet) {
            object["actions"] = JsonValueConverter<std::shared_ptr<Link>>::toJsonValue(getActions());
        }
        if (m_RunsIsSet) {
            object["runs"] = JsonValueConverter<std::shared_ptr<Link>>::toJsonValue(getRuns());
        }
        if (m_QueueIsSet) {
            object["queue"] = JsonValueConverter<std::shared_ptr<Link>>::toJsonValue(getQueue());
        }
        if (m__classIsSet) {
...
void PipelineImpllinks::fromJsonObject_internal(boost::json::object const& object)
{
    m_SelfIsSet = false;
    ...
    m__classIsSet = false;
    {
        const auto SelfIt = object.find("self");
        if (SelfIt != object.end()) {
            setSelf(JsonValueConverter<std::shared_ptr<Link>>::fromJsonValue(SelfIt->value()));
        }
    }
    {
        const auto ActionsIt = object.find("actions");
        if (ActionsIt != object.end()) {
            setActions(JsonValueConverter<std::shared_ptr<Link>>::fromJsonValue(ActionsIt->value()));
        }
    }
    {
        const auto RunsIt = object.find("runs");
        if (RunsIt != object.end()) {
            setRuns(JsonValueConverter<std::shared_ptr<Link>>::fromJsonValue(RunsIt->value()));
        }
    }
    {
        const auto QueueIt = object.find("queue");
        if (QueueIt != object.end()) {
            setQueue(JsonValueConverter<std::shared_ptr<Link>>::fromJsonValue(QueueIt->value()));
        }
    }
    {
...
std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<PipelineImpllinks>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<PipelineImpllinks>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<PipelineImpllinks>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<PipelineImpllinks>>>::fromJsonValue(boost::json::parse(json));
}
