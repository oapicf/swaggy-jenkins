...
Link::Link(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string Link::toJsonString(bool prettyJson /* = false */) const
{
...
boost::json::value Link::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void Link::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for Link");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object Link::toJsonObject_internal() const
{
    boost::json::object object;
        ...
        if (m_HrefIsSet) {
            object["href"] = JsonValueConverter<std::string>::toJsonValue(getHref());
        }
    return object;
}

void Link::fromJsonObject_internal(boost::json::object const& object)
{
    m__classIsSet = false;
    m_HrefIsSet = false;
    {
    ...
    {
        const auto HrefIt = object.find("href");
        if (HrefIt != object.end()) {
            setHref(JsonValueConverter<std::string>::fromJsonValue(HrefIt->value()));
        }
    }
}
...
void Link::setHref(std::string value)
{
    ...
    m_Href = std::move(value);
    m_HrefIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<Link>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<Link>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<Link>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<Link>>>::fromJsonValue(boost::json::parse(json));
}
