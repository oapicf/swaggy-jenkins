/**
 * Swaggy Jenkins
 * Jenkins API clients generated from Swagger / Open API specification
 *
 * The version of the OpenAPI document: 3.4.1-pre.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.24.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */



#include "QueueBlockedItem.h"

#include <cstddef>
#include <map>
#include <memory>
#include <stdexcept>
#include <sstream>
#include <string>
#include <iterator>
#include <utility>
#include <vector>
#include <boost/json.hpp>

namespace org {
namespace openapitools {
namespace client {
namespace model {

namespace {

template <typename Target>
struct JsonValueConverter
{
    static boost::json::value toJsonValue(const Target& sourceValue)
    {
        return boost::json::value_from(sourceValue);
    }

    static Target fromJsonValue(const boost::json::value& jsonValue)
    {
        return boost::json::value_to<Target>(jsonValue);
    }
};

template <>
struct JsonValueConverter<std::nullptr_t>
{
    static boost::json::value toJsonValue(std::nullptr_t)
    {
        return nullptr;
    }

    static std::nullptr_t fromJsonValue(const boost::json::value& jsonValue)
    {
        if (!jsonValue.is_null()) {
            throw std::invalid_argument("Expected a null JSON value");
        }
        return nullptr;
    }
};

template <typename ModelType>
struct JsonValueConverter<std::shared_ptr<ModelType>>
{
    static boost::json::value toJsonValue(const std::shared_ptr<ModelType>& model)
    {
        return model == nullptr ? boost::json::value(nullptr) : model->toJsonValue();
    }

    static std::shared_ptr<ModelType> fromJsonValue(const boost::json::value& jsonValue)
    {
        if (jsonValue.is_null()) {
            return nullptr;
        }
        return std::make_shared<ModelType>(jsonValue);
    }
};

template <typename Element>
struct JsonValueConverter<std::vector<Element>>
{
    static boost::json::value toJsonValue(const std::vector<Element>& sourceValues)
    {
        boost::json::array jsonValues;
        for (const auto& sourceValue : sourceValues) {
            jsonValues.emplace_back(JsonValueConverter<Element>::toJsonValue(sourceValue));
        }
        return jsonValues;
    }

    static std::vector<Element> fromJsonValue(const boost::json::value& jsonValue)
    {
        const auto& jsonValues = jsonValue.as_array();
        std::vector<Element> convertedValues;
        convertedValues.reserve(jsonValues.size());
        for (const auto& jsonElement : jsonValues) {
            convertedValues.emplace_back(JsonValueConverter<Element>::fromJsonValue(jsonElement));
        }
        return convertedValues;
    }
};

template <typename MappedValue>
struct JsonValueConverter<std::map<std::string, MappedValue>>
{
    static boost::json::value toJsonValue(const std::map<std::string, MappedValue>& sourceValues)
    {
        boost::json::object jsonValues;
        for (const auto& sourceEntry : sourceValues) {
            jsonValues[sourceEntry.first] = JsonValueConverter<MappedValue>::toJsonValue(sourceEntry.second);
        }
        return jsonValues;
    }

    static std::map<std::string, MappedValue> fromJsonValue(const boost::json::value& jsonValue)
    {
        std::map<std::string, MappedValue> convertedValues;
        for (const auto& jsonEntry : jsonValue.as_object()) {
            const std::string entryKey(jsonEntry.key().data(), jsonEntry.key().size());
            convertedValues.emplace(entryKey, JsonValueConverter<MappedValue>::fromJsonValue(jsonEntry.value()));
        }
        return convertedValues;
    }
};

void writePrettyJson(std::ostream& output, boost::json::value const& value, std::string& indent)
{
    if (value.is_object()) {
        const auto& object = value.as_object();
        if (object.empty()) {
            output << "{}";
            return;
        }

        output << "{\n";
        indent.append(4, ' ');
        for (auto iterator = object.begin(); iterator != object.end(); ++iterator) {
            output << indent << boost::json::serialize(iterator->key()) << ": ";
            writePrettyJson(output, iterator->value(), indent);
            if (std::next(iterator) != object.end()) {
                output << ',';
            }
            output << '\n';
        }
        indent.resize(indent.size() - 4);
        output << indent << '}';
        return;
    }

    if (value.is_array()) {
        const auto& array = value.as_array();
        if (array.empty()) {
            output << "[]";
            return;
        }

        output << "[\n";
        indent.append(4, ' ');
        for (auto iterator = array.begin(); iterator != array.end(); ++iterator) {
            output << indent;
            writePrettyJson(output, *iterator, indent);
            if (std::next(iterator) != array.end()) {
                output << ',';
            }
            output << '\n';
        }
        indent.resize(indent.size() - 4);
        output << indent << ']';
        return;
    }

    output << boost::json::serialize(value);
}
}

QueueBlockedItem::QueueBlockedItem(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string QueueBlockedItem::toJsonString(bool prettyJson /* = false */) const
{
    const auto value = toJsonValue();
    if (!prettyJson) {
        return boost::json::serialize(value);
    }

    std::ostringstream output;
    std::string indent;
    writePrettyJson(output, value, indent);
    return output.str();
}

void QueueBlockedItem::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value QueueBlockedItem::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void QueueBlockedItem::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for QueueBlockedItem");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object QueueBlockedItem::toJsonObject_internal() const
{
    boost::json::object object;
        if (m__classIsSet) {
            object["_class"] = JsonValueConverter<std::string>::toJsonValue(get_Class());
        }
        if (m_ActionsIsSet) {
            object["actions"] = JsonValueConverter<std::vector<std::shared_ptr<CauseAction>>>::toJsonValue(getActions());
        }
        if (m_BlockedIsSet) {
            object["blocked"] = JsonValueConverter<bool>::toJsonValue(isBlocked());
        }
        if (m_BuildableIsSet) {
            object["buildable"] = JsonValueConverter<bool>::toJsonValue(isBuildable());
        }
        if (m_IdIsSet) {
            object["id"] = JsonValueConverter<int32_t>::toJsonValue(getId());
        }
        if (m_InQueueSinceIsSet) {
            object["inQueueSince"] = JsonValueConverter<int32_t>::toJsonValue(getInQueueSince());
        }
        if (m_ParamsIsSet) {
            object["params"] = JsonValueConverter<std::string>::toJsonValue(getParams());
        }
        if (m_StuckIsSet) {
            object["stuck"] = JsonValueConverter<bool>::toJsonValue(isStuck());
        }
        if (m_TaskIsSet) {
            object["task"] = JsonValueConverter<std::shared_ptr<FreeStyleProject>>::toJsonValue(getTask());
        }
        if (m_UrlIsSet) {
            object["url"] = JsonValueConverter<std::string>::toJsonValue(getUrl());
        }
        if (m_WhyIsSet) {
            object["why"] = JsonValueConverter<std::string>::toJsonValue(getWhy());
        }
        if (m_BuildableStartMillisecondsIsSet) {
            object["buildableStartMilliseconds"] = JsonValueConverter<int32_t>::toJsonValue(getBuildableStartMilliseconds());
        }
    return object;
}

void QueueBlockedItem::fromJsonObject_internal(boost::json::object const& object)
{
    m__classIsSet = false;
    m_ActionsIsSet = false;
    m_BlockedIsSet = false;
    m_BuildableIsSet = false;
    m_IdIsSet = false;
    m_InQueueSinceIsSet = false;
    m_ParamsIsSet = false;
    m_StuckIsSet = false;
    m_TaskIsSet = false;
    m_UrlIsSet = false;
    m_WhyIsSet = false;
    m_BuildableStartMillisecondsIsSet = false;
    {
        const auto _classIt = object.find("_class");
        if (_classIt != object.end()) {
            set_Class(JsonValueConverter<std::string>::fromJsonValue(_classIt->value()));
        }
    }
    {
        const auto ActionsIt = object.find("actions");
        if (ActionsIt != object.end()) {
            setActions(JsonValueConverter<std::vector<std::shared_ptr<CauseAction>>>::fromJsonValue(ActionsIt->value()));
        }
    }
    {
        const auto BlockedIt = object.find("blocked");
        if (BlockedIt != object.end()) {
            setBlocked(JsonValueConverter<bool>::fromJsonValue(BlockedIt->value()));
        }
    }
    {
        const auto BuildableIt = object.find("buildable");
        if (BuildableIt != object.end()) {
            setBuildable(JsonValueConverter<bool>::fromJsonValue(BuildableIt->value()));
        }
    }
    {
        const auto IdIt = object.find("id");
        if (IdIt != object.end()) {
            setId(JsonValueConverter<int32_t>::fromJsonValue(IdIt->value()));
        }
    }
    {
        const auto InQueueSinceIt = object.find("inQueueSince");
        if (InQueueSinceIt != object.end()) {
            setInQueueSince(JsonValueConverter<int32_t>::fromJsonValue(InQueueSinceIt->value()));
        }
    }
    {
        const auto ParamsIt = object.find("params");
        if (ParamsIt != object.end()) {
            setParams(JsonValueConverter<std::string>::fromJsonValue(ParamsIt->value()));
        }
    }
    {
        const auto StuckIt = object.find("stuck");
        if (StuckIt != object.end()) {
            setStuck(JsonValueConverter<bool>::fromJsonValue(StuckIt->value()));
        }
    }
    {
        const auto TaskIt = object.find("task");
        if (TaskIt != object.end()) {
            setTask(JsonValueConverter<std::shared_ptr<FreeStyleProject>>::fromJsonValue(TaskIt->value()));
        }
    }
    {
        const auto UrlIt = object.find("url");
        if (UrlIt != object.end()) {
            setUrl(JsonValueConverter<std::string>::fromJsonValue(UrlIt->value()));
        }
    }
    {
        const auto WhyIt = object.find("why");
        if (WhyIt != object.end()) {
            setWhy(JsonValueConverter<std::string>::fromJsonValue(WhyIt->value()));
        }
    }
    {
        const auto BuildableStartMillisecondsIt = object.find("buildableStartMilliseconds");
        if (BuildableStartMillisecondsIt != object.end()) {
            setBuildableStartMilliseconds(JsonValueConverter<int32_t>::fromJsonValue(BuildableStartMillisecondsIt->value()));
        }
    }
}

std::string QueueBlockedItem::get_Class() const
{
    return m__class;
}

void QueueBlockedItem::set_Class(std::string value)
{
    
    m__class = std::move(value);
    m__classIsSet = true;
}
std::vector<std::shared_ptr<CauseAction>> QueueBlockedItem::getActions() const
{
    return m_Actions;
}

void QueueBlockedItem::setActions(std::vector<std::shared_ptr<CauseAction>> value)
{
    
    m_Actions = std::move(value);
    m_ActionsIsSet = true;
}
bool QueueBlockedItem::isBlocked() const
{
    return m_Blocked;
}

void QueueBlockedItem::setBlocked(bool value)
{
    
    m_Blocked = std::move(value);
    m_BlockedIsSet = true;
}
bool QueueBlockedItem::isBuildable() const
{
    return m_Buildable;
}

void QueueBlockedItem::setBuildable(bool value)
{
    
    m_Buildable = std::move(value);
    m_BuildableIsSet = true;
}
int32_t QueueBlockedItem::getId() const
{
    return m_Id;
}

void QueueBlockedItem::setId(int32_t value)
{
    
    m_Id = std::move(value);
    m_IdIsSet = true;
}
int32_t QueueBlockedItem::getInQueueSince() const
{
    return m_InQueueSince;
}

void QueueBlockedItem::setInQueueSince(int32_t value)
{
    
    m_InQueueSince = std::move(value);
    m_InQueueSinceIsSet = true;
}
std::string QueueBlockedItem::getParams() const
{
    return m_Params;
}

void QueueBlockedItem::setParams(std::string value)
{
    
    m_Params = std::move(value);
    m_ParamsIsSet = true;
}
bool QueueBlockedItem::isStuck() const
{
    return m_Stuck;
}

void QueueBlockedItem::setStuck(bool value)
{
    
    m_Stuck = std::move(value);
    m_StuckIsSet = true;
}
std::shared_ptr<FreeStyleProject> QueueBlockedItem::getTask() const
{
    return m_Task;
}

void QueueBlockedItem::setTask(std::shared_ptr<FreeStyleProject> value)
{
    
    m_Task = std::move(value);
    m_TaskIsSet = true;
}
std::string QueueBlockedItem::getUrl() const
{
    return m_Url;
}

void QueueBlockedItem::setUrl(std::string value)
{
    
    m_Url = std::move(value);
    m_UrlIsSet = true;
}
std::string QueueBlockedItem::getWhy() const
{
    return m_Why;
}

void QueueBlockedItem::setWhy(std::string value)
{
    
    m_Why = std::move(value);
    m_WhyIsSet = true;
}
int32_t QueueBlockedItem::getBuildableStartMilliseconds() const
{
    return m_BuildableStartMilliseconds;
}

void QueueBlockedItem::setBuildableStartMilliseconds(int32_t value)
{
    
    m_BuildableStartMilliseconds = std::move(value);
    m_BuildableStartMillisecondsIsSet = true;
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<QueueBlockedItem>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<QueueBlockedItem>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<QueueBlockedItem>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<QueueBlockedItem>>>::fromJsonValue(boost::json::parse(json));
}

}
}
}
}

