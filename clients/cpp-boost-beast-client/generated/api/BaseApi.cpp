...
BaseApiException::BaseApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status BaseApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<DefaultCrumbIssuer>
BaseApi::getCrumb(
    ) {
    std::string serializedRequestBody;
    std::string path = m_context + "/crumbIssuer/api/json";
    ...
    std::shared_ptr<DefaultCrumbIssuer> deserializedResponse = std::make_shared<DefaultCrumbIssuer>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<DefaultCrumbIssuer>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(401)) {
        throw BaseApiException(statusCode, "Authentication failed - incorrect username and/or password");
    }
    if (statusCode == boost::beast::http::status(403)) {
        throw BaseApiException(statusCode, "Jenkins requires authentication - please set username and password");
    }
    throw BaseApiException(statusCode, "Unexpected HTTP status code");
}


std::string BaseApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void BaseApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void BaseApi::handleStdException(const std::exception&) {
    throw;
}

void BaseApi::handleUncaughtException() {
    throw;
}
