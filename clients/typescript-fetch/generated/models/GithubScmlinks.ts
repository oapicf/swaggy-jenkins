/* tslint:disable */
/* eslint-disable */
/**
 * Swaggy Jenkins
 * Jenkins API clients generated from Swagger / Open API specification
 *
 * The version of the OpenAPI document: 3.4.1-pre.0
 * Contact: blah+oapicf@cliffano.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { Link } from './Link';
import {
    LinkFromJSON,
    LinkFromJSONTyped,
    LinkToJSON,
    LinkToJSONTyped,
} from './Link';

/**
 * 
 * @export
 * @interface GithubScmlinks
 */
export interface GithubScmlinks {
    /**
     * 
     * @type {Link}
     * @memberof GithubScmlinks
     */
    self?: Link;
    /**
     * 
     * @type {string}
     * @memberof GithubScmlinks
     */
    _class?: string;
}

/**
 * Check if a given object implements the GithubScmlinks interface.
 */
export function instanceOfGithubScmlinks(value: object): value is GithubScmlinks {
    return true;
}

export function GithubScmlinksFromJSON(json: any): GithubScmlinks {
    return GithubScmlinksFromJSONTyped(json, false);
}

export function GithubScmlinksFromJSONTyped(json: any, ignoreDiscriminator: boolean): GithubScmlinks {
    if (json == null) {
        return json;
    }
    return {
        
        'self': json['self'] == null ? undefined : LinkFromJSON(json['self']),
        '_class': json['_class'] == null ? undefined : json['_class'],
    };
}

export function GithubScmlinksToJSON(json: any): GithubScmlinks {
    return GithubScmlinksToJSONTyped(json, false);
}

export function GithubScmlinksToJSONTyped(value?: GithubScmlinks | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'self': LinkToJSON(value['self']),
        '_class': value['_class'],
    };
}

