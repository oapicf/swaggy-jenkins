...
# Custom JSON deserialization for QueueBlockedItem with custom field names
proc to*(node: JsonNode, T: typedesc[QueueBlockedItem]): QueueBlockedItem =
  result = QueueBlockedItem()
  if node.kind == JObject:
    ...
    if node.hasKey("buildableStartMilliseconds") and node["buildableStartMilliseconds"].kind != JNull:
      result.buildableStartMilliseconds = some(to(node["buildableStartMilliseconds"], typeof(result.buildableStartMilliseconds.get())))

# Custom JSON serialization for QueueBlockedItem with custom field names
proc `%`*(obj: QueueBlockedItem): JsonNode =
  result = newJObject()
  if obj.class.isSome():
  ...
  if obj.buildableStartMilliseconds.isSome():
    result["buildableStartMilliseconds"] = %obj.buildableStartMilliseconds.get()
