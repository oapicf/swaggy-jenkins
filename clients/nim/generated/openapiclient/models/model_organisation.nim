...
# Custom JSON deserialization for Organisation with custom field names
proc to*(node: JsonNode, T: typedesc[Organisation]): Organisation =
  result = Organisation()
...
# Custom JSON serialization for Organisation with custom field names
proc `%`*(obj: Organisation): JsonNode =
  result = newJObject()
  if obj.class.isSome():
