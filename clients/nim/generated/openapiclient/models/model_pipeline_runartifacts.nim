...
type PipelineRunartifacts* = object
  ## 
  name*: Option[string]
  size*: Option[int]
  url*: Option[string]
...
# Custom JSON deserialization for PipelineRunartifacts with custom field names
proc to*(node: JsonNode, T: typedesc[PipelineRunartifacts]): PipelineRunartifacts =
  result = PipelineRunartifacts()
...
# Custom JSON serialization for PipelineRunartifacts with custom field names
proc `%`*(obj: PipelineRunartifacts): JsonNode =
  result = newJObject()
  if obj.name.isSome():
