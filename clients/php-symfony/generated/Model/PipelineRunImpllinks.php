<?php
/**
 * PipelineRunImpllinks
 *
 * PHP version 8.1.1
 *
 * @category Class
 * @package  OpenAPI\Server\Model
 * @author   OpenAPI Generator team
 * @link     https://github.com/openapitools/openapi-generator
 */

/**
 * Swaggy Jenkins
 *
 * Jenkins API clients generated from Swagger / Open API specification
 *
 * The version of the OpenAPI document: 3.4.1-pre.0
 * Contact: blah+oapicf@cliffano.com
 * Generated by: https://github.com/openapitools/openapi-generator.git
 *
 */

/**
 * NOTE: This class is auto generated by the openapi generator program.
 * https://github.com/openapitools/openapi-generator
 * Do not edit the class manually.
 */

namespace OpenAPI\Server\Model;

use Symfony\Component\Validator\Constraints as Assert;
use JMS\Serializer\Annotation\Type;
use JMS\Serializer\Annotation\Accessor;
use JMS\Serializer\Annotation\SerializedName;

/**
 * Class representing the PipelineRunImpllinks model.
 *
 * @package OpenAPI\Server\Model
 * @author  OpenAPI Generator team
 */

class PipelineRunImpllinks 
{
        /**
     * @var Link|null
     * @SerializedName("nodes")
     * @Type("OpenAPI\Server\Model\Link")
     */
    #[Assert\Type("OpenAPI\Server\Model\Link")]
    protected ?Link $nodes = null;

    /**
     * @var Link|null
     * @SerializedName("log")
     * @Type("OpenAPI\Server\Model\Link")
     */
    #[Assert\Type("OpenAPI\Server\Model\Link")]
    protected ?Link $log = null;

    /**
     * @var Link|null
     * @SerializedName("self")
     * @Type("OpenAPI\Server\Model\Link")
     */
    #[Assert\Type("OpenAPI\Server\Model\Link")]
    protected ?Link $self = null;

    /**
     * @var Link|null
     * @SerializedName("actions")
     * @Type("OpenAPI\Server\Model\Link")
     */
    #[Assert\Type("OpenAPI\Server\Model\Link")]
    protected ?Link $actions = null;

    /**
     * @var Link|null
     * @SerializedName("steps")
     * @Type("OpenAPI\Server\Model\Link")
     */
    #[Assert\Type("OpenAPI\Server\Model\Link")]
    protected ?Link $steps = null;

    /**
     * @var string|null
     * @SerializedName("_class")
     * @Type("string")
     */
    #[Assert\Type("string")]
    protected ?string $class = null;

    /**
     * Constructor
     * @param array|null $data Associated array of property values initializing the model
     */
    public function __construct(?array $data = null)
    {
        if (is_array($data)) {
            $this->nodes = array_key_exists('nodes', $data) ? $data['nodes'] : $this->nodes;
            $this->log = array_key_exists('log', $data) ? $data['log'] : $this->log;
            $this->self = array_key_exists('self', $data) ? $data['self'] : $this->self;
            $this->actions = array_key_exists('actions', $data) ? $data['actions'] : $this->actions;
            $this->steps = array_key_exists('steps', $data) ? $data['steps'] : $this->steps;
            $this->class = array_key_exists('class', $data) ? $data['class'] : $this->class;
        }
    }

    /**
     * Gets nodes.
     *
     * @return Link|null
     */
    public function getNodes(): ?Link
    {
        return $this->nodes;
    }

    /**
     * Sets nodes.
     *
     * @param Link|null $nodes
     *
     * @return $this
     */
    public function setNodes(?Link $nodes = null): self
    {
        $this->nodes = $nodes;

        return $this;
    }




    /**
     * Gets log.
     *
     * @return Link|null
     */
    public function getLog(): ?Link
    {
        return $this->log;
    }

    /**
     * Sets log.
     *
     * @param Link|null $log
     *
     * @return $this
     */
    public function setLog(?Link $log = null): self
    {
        $this->log = $log;

        return $this;
    }




    /**
     * Gets self.
     *
     * @return Link|null
     */
    public function getSelf(): ?Link
    {
        return $this->self;
    }

    /**
     * Sets self.
     *
     * @param Link|null $self
     *
     * @return $this
     */
    public function setSelf(?Link $self = null): self
    {
        $this->self = $self;

        return $this;
    }




    /**
     * Gets actions.
     *
     * @return Link|null
     */
    public function getActions(): ?Link
    {
        return $this->actions;
    }

    /**
     * Sets actions.
     *
     * @param Link|null $actions
     *
     * @return $this
     */
    public function setActions(?Link $actions = null): self
    {
        $this->actions = $actions;

        return $this;
    }




    /**
     * Gets steps.
     *
     * @return Link|null
     */
    public function getSteps(): ?Link
    {
        return $this->steps;
    }

    /**
     * Sets steps.
     *
     * @param Link|null $steps
     *
     * @return $this
     */
    public function setSteps(?Link $steps = null): self
    {
        $this->steps = $steps;

        return $this;
    }




    /**
     * Gets class.
     *
     * @return string|null
     */
    public function getClass(): ?string
    {
        return $this->class;
    }

    /**
     * Sets class.
     *
     * @param string|null $class
     *
     * @return $this
     */
    public function setClass(?string $class = null): self
    {
        $this->class = $class;

        return $this;
    }



}


