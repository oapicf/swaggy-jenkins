...
feature -- API Access
	...
	json_web_token (expiry_time_in_mins: INTEGER_32; max_expiry_time_in_mins: INTEGER_32): detachable STRING_32
			...
			-- 
			-- argument: expiry_time_in_mins Token expiry time in minutes, default: 30 minutes (optional)
			-- 
			-- argument: max_expiry_time_in_mins Maximum token expiry time in minutes, default: 480 minutes (optional)
			-- 
			-- 
			-- Result STRING_32
		require
	...
	organisations : detachable ORGANISATIONS
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { ORGANISATIONS } l_response.data ({ ORGANISATIONS }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	pipeline_activities (organization: STRING_32; pipeline: STRING_32): detachable PIPELINE_ACTIVITIES
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { PIPELINE_ACTIVITIES } l_response.data ({ PIPELINE_ACTIVITIES }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	pipeline_queue (organization: STRING_32; pipeline: STRING_32): detachable PIPELINE_QUEUE
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { PIPELINE_QUEUE } l_response.data ({ PIPELINE_QUEUE }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	pipeline_run_log (organization: STRING_32; pipeline: STRING_32; run: STRING_32; start: INTEGER_32; download: BOOLEAN): detachable STRING_32
			...
			-- argument: run Name of the run (required)
			-- 
			-- argument: start Start position of the log (optional)
			-- 
			-- argument: download Set to true in order to download the file, otherwise it&#39;s passed as a response body (optional)
			-- 
			-- 
			-- Result STRING_32
		require
	...
	pipeline_run_node_steps (organization: STRING_32; pipeline: STRING_32; run: STRING_32; node: STRING_32): detachable PIPELINE_RUN_NODE_STEPS
			...
			-- 
			-- 
			-- Result PIPELINE_RUN_NODE_STEPS
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { PIPELINE_RUN_NODE_STEPS } l_response.data ({ PIPELINE_RUN_NODE_STEPS }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	pipeline_run_nodes (organization: STRING_32; pipeline: STRING_32; run: STRING_32): detachable PIPELINE_RUN_NODES
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { PIPELINE_RUN_NODES } l_response.data ({ PIPELINE_RUN_NODES }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	pipeline_runs (organization: STRING_32; pipeline: STRING_32): detachable PIPELINE_RUNS
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { PIPELINE_RUNS } l_response.data ({ PIPELINE_RUNS }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	pipelines (organization: STRING_32): detachable PIPELINES
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { PIPELINES } l_response.data ({ PIPELINES }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	put_pipeline_run (organization: STRING_32; pipeline: STRING_32; run: STRING_32; blocking: STRING_32; time_out_in_secs: INTEGER_32): detachable PIPELINE_RUN
			...
			-- argument: run Name of the run (required)
			-- 
			-- argument: blocking Set to true to make blocking stop, default: false (optional)
			-- 
			-- argument: time_out_in_secs Timeout in seconds, default: 10 seconds (optional)
			-- 
			-- 
			-- Result PIPELINE_RUN
		require
	...
	sc_morganisation_repositories (organization: STRING_32; scm: STRING_32; scm_organisation: STRING_32; credential_id: STRING_32; page_size: INTEGER_32; page_number: INTEGER_32): detachable SCM_ORGANISATIONS
			-- 
			...
			-- 
			-- argument: credential_id Credential ID (optional)
			-- 
			-- argument: page_size Number of items in a page (optional)
			-- 
			-- argument: page_number Page number (optional)
			-- 
			-- 
			-- Result SCM_ORGANISATIONS
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { SCM_ORGANISATIONS } l_response.data ({ SCM_ORGANISATIONS }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	sc_morganisation_repository (organization: STRING_32; scm: STRING_32; scm_organisation: STRING_32; repository: STRING_32; credential_id: STRING_32): detachable SCM_ORGANISATIONS
			-- 
	...
	sc_morganisations (organization: STRING_32; scm: STRING_32; credential_id: STRING_32): detachable SCM_ORGANISATIONS
			-- 
	...
	user_favorites (user: STRING_32): detachable USER_FAVORITES
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { USER_FAVORITES } l_response.data ({ USER_FAVORITES }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
