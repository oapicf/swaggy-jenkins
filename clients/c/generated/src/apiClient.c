...
apiClient_t *apiClient_create() {
    ...
    apiClient->apiKeys_jwt_auth = NULL;
...
, list_t *apiKeys_jwt_auth
    ...
    apiClient->username = NULL;
    apiClient->password = NULL;
    if(apiKeys_jwt_auth!= NULL) {
        apiClient->apiKeys_jwt_auth = list_createList();
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiKeys_jwt_auth) {
            keyValuePair_t *pair = listEntry->data;
            keyValuePair_t *pairDup = keyValuePair_create(strdup(pair->key), strdup(pair->value));
            list_addElement(apiClient->apiKeys_jwt_auth, pairDup);
        }
    }else{
        apiClient->apiKeys_jwt_auth = NULL;
    }
...
void apiClient_free(apiClient_t *apiClient) {
    ...
    if(apiClient->apiKeys_jwt_auth) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiClient->apiKeys_jwt_auth) {
            keyValuePair_t *pair = listEntry->data;
            if(pair->key){
                free(pair->key);
            }
        ...
        list_freeList(apiClient->apiKeys_jwt_auth);
    }
    free(apiClient);
}
...
void apiClient_invoke(apiClient_t    *apiClient,
    ...
    if(handle) {
        ...
        if (apiClient->apiKeys_jwt_auth != NULL)
        {
        list_ForEach(listEntry, apiClient->apiKeys_jwt_auth) {
