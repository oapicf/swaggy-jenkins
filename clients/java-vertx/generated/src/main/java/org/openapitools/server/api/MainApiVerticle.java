...
public class MainApiVerticle extends AbstractVerticle {
    ...
    public void deployVerticles(Future<Void> startFuture) {
        ...
        vertx.deployVerticle("org.openapitools.server.api.verticle.BaseAccessApiVerticle", res -> {
            if (res.succeeded()) {
                LOGGER.info("BaseAccessApiVerticle : Deployed");
            } else {
                startFuture.fail(res.cause());
                LOGGER.error("BaseAccessApiVerticle : Deployment failed");
            }
        });
        ...
        vertx.deployVerticle("org.openapitools.server.api.verticle.BlueOceanApiVerticle", res -> {
            if (res.succeeded()) {
                ...
                startFuture.fail(res.cause());
                LOGGER.error("BlueOceanApiVerticle : Deployment failed");
            }
        });
        
        vertx.deployVerticle("org.openapitools.server.api.verticle.RemoteAccessApiVerticle", res -> {
            if (res.succeeded()) {
                LOGGER.info("RemoteAccessApiVerticle : Deployed");
            } else {
                startFuture.fail(res.cause());
                LOGGER.error("RemoteAccessApiVerticle : Deployment failed");
            }
        });
