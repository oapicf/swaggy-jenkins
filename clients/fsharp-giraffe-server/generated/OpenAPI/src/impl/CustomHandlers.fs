...
module CustomHandlers =
  ...
  let loginView =
    html [] [
        ...
        body [] [
            h1 [] [ str "Welcome" ]
            a [_href "/login-with-jenkins_auth"] [ str "Login with jenkins_auth" ]
            a [_href "/login-with-jwt_auth"] [ str "Login with jwt_auth" ]
        ]
    ]
  ...
  let handlers : HttpHandler list = [
    ...
    GET >=>
      choose [
        ...
        route "/login-with-jenkins_auth" >=> challenge "jenkins_auth"
        route "/login-with-jwt_auth" >=> challenge "jwt_auth"
        route "/logout" >=> logout
      ]
  ]
