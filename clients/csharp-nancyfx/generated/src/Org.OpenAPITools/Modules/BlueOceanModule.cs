...
{ 
    ...
    public sealed class BlueOceanModule : NancyModule
    {
        ...
        { 
            ...
            Get["/jwt-auth/jwks/{key}"] = parameters =>
            {
                var key = Parameters.ValueOf<int?>(parameters, Context.Request, "key", ParameterType.Path);
                Preconditions.IsNotNull(key, "Required parameter: 'key' is missing at 'GetJsonWebKey'");
                
                return service.GetJsonWebKey(Context, key);
            };

            Get["/jwt-auth/token"] = parameters =>
            {
                var expiryTimeInMins = Parameters.ValueOf<int?>(parameters, Context.Request, "expiryTimeInMins", ParameterType.Query);
                var maxExpiryTimeInMins = Parameters.ValueOf<int?>(parameters, Context.Request, "maxExpiryTimeInMins", ParameterType.Query);
                return service.GetJsonWebToken(Context, expiryTimeInMins, maxExpiryTimeInMins);
            };
    ...
    public interface BlueOceanService
    {
        ...
        /// <returns>string</returns>
        string GetJsonWebKey(NancyContext context, int? key);
        ...
        /// <returns>string</returns>
        string GetJsonWebToken(NancyContext context, int? expiryTimeInMins, int? maxExpiryTimeInMins);
    ...
    public abstract class AbstractBlueOceanService: BlueOceanService
    {
        ...
        public virtual string GetJsonWebKey(NancyContext context, int? key)
        {
            return GetJsonWebKey(key);
        }

        public virtual string GetJsonWebToken(NancyContext context, int? expiryTimeInMins, int? maxExpiryTimeInMins)
        {
            return GetJsonWebToken(expiryTimeInMins, maxExpiryTimeInMins);
        }
        ...
        protected abstract string GetJsonWebKey(int? key);

        protected abstract string GetJsonWebToken(int? expiryTimeInMins, int? maxExpiryTimeInMins);
