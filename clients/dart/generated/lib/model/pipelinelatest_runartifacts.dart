...
class PipelinelatestRunartifacts {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PipelinelatestRunartifacts[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PipelinelatestRunartifacts[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PipelinelatestRunartifacts> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PipelinelatestRunartifacts>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PipelinelatestRunartifacts.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PipelinelatestRunartifacts> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = PipelinelatestRunartifacts.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<PipelinelatestRunartifacts>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PipelinelatestRunartifacts>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = PipelinelatestRunartifacts.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
