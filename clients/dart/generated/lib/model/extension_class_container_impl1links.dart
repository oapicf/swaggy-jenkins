...
class ExtensionClassContainerImpl1links {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ExtensionClassContainerImpl1links[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ExtensionClassContainerImpl1links[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ExtensionClassContainerImpl1links> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ExtensionClassContainerImpl1links>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ExtensionClassContainerImpl1links.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ExtensionClassContainerImpl1links> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = ExtensionClassContainerImpl1links.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<ExtensionClassContainerImpl1links>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ExtensionClassContainerImpl1links>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ExtensionClassContainerImpl1links.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
