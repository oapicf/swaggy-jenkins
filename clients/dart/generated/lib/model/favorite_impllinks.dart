...
class FavoriteImpllinks {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FavoriteImpllinks[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FavoriteImpllinks[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<FavoriteImpllinks> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <FavoriteImpllinks>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = FavoriteImpllinks.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, FavoriteImpllinks> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = FavoriteImpllinks.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<FavoriteImpllinks>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FavoriteImpllinks>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = FavoriteImpllinks.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
