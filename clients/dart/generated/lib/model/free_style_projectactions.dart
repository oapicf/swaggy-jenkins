    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FreeStyleProjectactions[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FreeStyleProjectactions[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<FreeStyleProjectactions> listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = FreeStyleProjectactions.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, FreeStyleProjectactions> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = FreeStyleProjectactions.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
