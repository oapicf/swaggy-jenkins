...
class GithubFile {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "GithubFile[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "GithubFile[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<GithubFile> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <GithubFile>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = GithubFile.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, GithubFile> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = GithubFile.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<GithubFile>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<GithubFile>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = GithubFile.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
