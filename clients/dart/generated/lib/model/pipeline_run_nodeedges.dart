...
class PipelineRunNodeedges {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PipelineRunNodeedges[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PipelineRunNodeedges[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PipelineRunNodeedges> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PipelineRunNodeedges>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PipelineRunNodeedges.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PipelineRunNodeedges> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = PipelineRunNodeedges.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<PipelineRunNodeedges>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PipelineRunNodeedges>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = PipelineRunNodeedges.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
