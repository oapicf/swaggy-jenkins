    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FreeStyleProjecthealthReport[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FreeStyleProjecthealthReport[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<FreeStyleProjecthealthReport> listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = FreeStyleProjecthealthReport.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, FreeStyleProjecthealthReport> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = FreeStyleProjecthealthReport.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
