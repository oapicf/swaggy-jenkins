...
// AssertRecursePipelineRunImplRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of PipelineRunImpl (e.g. [][]PipelineRunImpl), otherwise ErrTypeAssertionError is thrown.
func AssertRecursePipelineRunImplRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aPipelineRunImpl, ok := obj.(PipelineRunImpl)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertPipelineRunImplRequired(aPipelineRunImpl)
	})
}
